...
pub fn codegen(
    ...
    for (name, task) in &app.software_tasks {
        ...
        if let Some(free_queues) = analysis.free_queues.get(name) {
            for (&_sender, &ceiling) in free_queues {
                let fq = util::fq_ident(name);

                let fq_ty = quote!(rtfm::export::SCFQ<#cap_ty>);
                let fq_expr = quote!(rtfm::export::Queue(unsafe {
                ...
                if app.uses_schedule(receiver) {
                    ...
                    const_app.push(quote!(
                        ...
                        static mut #instants:
                            [core::mem::MaybeUninit<rtfm::Instant>; #cap_lit] =
                            [#(#elems,)*];
                    ));
                }
